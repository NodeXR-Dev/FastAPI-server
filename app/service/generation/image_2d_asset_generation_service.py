...
class Image2DAssetGenerationService:
    ...
    async def generate(
        ...
        *,
        ...
        return await self.persist_generated_image(
            room_id=room_id,
            user_id=user_id,
        ...
        *,
        ...
        try:
            stored_object = await asyncio.to_thread(
                self.minio_asset_storage.upload_generated_image,
                room_id=room_id,
                graph_snapshot_id=graph_snapshot_id,
            ...
            if graph_snapshot_id is not None:
                updated_graph_snapshot = (
                    self.graph_repository.create_graph_snapshot_from_input_snapshot(
                        db=self.db,
                        room_id=room_id,
                        input_graph_snapshot_id=graph_snapshot_id,
            ...
            self.asset_repository.update_graph_snapshot(
                db=self.db,
                asset=asset,
            ...
            event_payload = {
                ...
                "graph_snapshot_id": str(updated_graph_snapshot.graph_snapshot_id),
            }
            event_payload.update(event_payload_extra or {})
            if event_payload.get("generation_type") == "COLOR_CHANGE":
                event_payload["generated_asset_id"] = str(asset.asset_id)
                event_payload["generated_asset_url"] = stored_object.public_url
