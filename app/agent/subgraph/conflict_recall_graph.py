...
from app.agent.prompt.conflict_recall_prompt import CONFLICT_RECALL_PROMPT
...
class ConflictRecallGraph:
    def __init__(
        ...
        *,
        ...
        self.response_chain = CONFLICT_RECALL_PROMPT | (llm or get_agent_llm())
        self.graph = self._compile()

    def _compile(self):
        builder = StateGraph(RealtimeAgentState)
        builder.add_node("retrieve_conflict", self.retrieve_conflict)
        builder.add_node("retrieve_arguments", self.retrieve_arguments)
        builder.add_node("generate_response", self.generate_response)
        builder.add_edge(START, "retrieve_conflict")
        builder.add_edge("retrieve_conflict", "retrieve_arguments")
        builder.add_edge("retrieve_arguments", "generate_response")
        builder.add_edge("generate_response", END)
        return builder.compile(name="ConflictRecall")

    async def retrieve_conflict(self, state: RealtimeAgentState) -> dict:
        try:
            facts = await asyncio.to_thread(
                self.retrieval_service.retrieve_conflict_seeds,
            ...
            logger.exception(
                "[conflict_retrieval_failed] room_id=%s | utterance_id=%s | error=%s",
                state["room_id"],
                state["utterance_id"],
                str(error),
            )
            return {"errors": ["conflict_retrieval_failed"]}

    async def retrieve_arguments(self, state: RealtimeAgentState) -> dict:
        ...
        try:
            ...
            logger.exception(
                "[conflict_argument_retrieval_failed] room_id=%s | utterance_id=%s | error=%s",
                state["room_id"],
                state["utterance_id"],
                str(error),
            )
    ...
    async def generate_response(self, state: RealtimeAgentState) -> dict:
        if not state["retrieved_facts"]:
            return {
                "responses": [
                    AgentResponse(
                        response_type="CONFLICT_RECALL",
                        message="저장된 논쟁 근거를 찾지 못했습니다.",
                    )
                ]
            }
        try:
            ...
            logger.exception(
                "[conflict_response_failed] room_id=%s | utterance_id=%s | error=%s",
                state["room_id"],
                state["utterance_id"],
                str(error),
            )
            return {"errors": ["conflict_response_failed"]}
