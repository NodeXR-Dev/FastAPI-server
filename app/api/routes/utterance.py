...
from app.services.utterances.button_utterance_service import ButtonUtteranceService
...
def create_node_from_utterance(
    ...
    stage = "api_create_node_utterance"
    start_time = time.perf_counter()

    logger.info(
        "[api_create_node_utterance] request_start | room_id=%s | user_id=%s | parent_node_id=%s | utterance_length=%s",
        request.room_id,
        request.user_id,
    ...
    logger.info(
        "[api_create_node_utterance] request_done | elapsed_ms=%.2f | avg_ms=%.2f | room_id=%s | graph_version=%s ",
        elapsed_ms,
        avg_ms,
        result.room_id,
